...
% Name.index=[];
% Name.errorb=[];                %计算前的mistake数量，初始假设全错
% Name.weriht=[];
...
while inter==true
    ...
    errora=0;                %计算后的mistake数量
    for j=1:size(x,2)        %循环遍历所有点 
        sign_v=sign(W(1)*x0+W(:,2:3)*x(1:2,j));  %计算分类的符号，默认W(1)=0,x0=1       
        if sign_v~=x(3,j)                       %分类符号与label不一致的情况下，对权重W进行修正 
            W(1)=W(1)+x(3,j)*1;                 %默认x0=1，更新W(1)的值
            W(:,2:3)=W(:,2:3)+x(3,j)*x(1:2,j)'; %根据w'=w+y*x更新W(2)和W(3)的值 
           errora = errora+1;                    %存在分类错误的点，error+1
        end      
    end
%     Name.index=[Name.index; inter_num];
%     Name.errorb=[Name.errorb;errora];         
%     Name.weriht=[Name.weriht;W];           
    if errora<errorH
        errorH = errora;
        WH=W;
    elseif inter_num==22   %enough iterations
...
y1=-WH(2)/WH(3)*x1-WH(1)/WH(3); %直线纵坐标值，由W(1)+W(2)x1+W(3)y1=0得到
plot(x1,y1,'-g');           %画出分类直线
